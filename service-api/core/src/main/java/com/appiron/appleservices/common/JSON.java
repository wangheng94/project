...
//        dateModule.addSerializer(Date.class, new DateSerializer(false, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")));
        ...
        build(withEmptyMapper);
    }

    /**
     * 设置 mappepr 的通用属性
     */
    private static void build(ObjectMapper mapper) {
    ...
    public static String writeAsPrettyJsonString(Object object) {
        try {
     ...
     * 进行类型转换
     */
    public static <T> T parseObject(Object origin, Class<T> targetClass) {
    ...
    public static String get(String json, String key) {
        ...
        if (StrUtil.isEmpty(json) || StrUtil.isEmpty(key)) {
            return null;
        }
    ...
    public static <T, E extends Collection> Collection<T> parseCollection(String json, Class<E> collectionClass, Class<T> valueType) {
        ...
        if (StrUtil.isEmpty(json) || valueType == null) {
            return null;
        }
    ...
    public static <T> T parse(Object obj, Class<T> type) {
        ...
        if (Objects.isNull(obj) || type == null) {
            return null;
        }
